...
namespace Api_Servixe.Controllers.V1
{
    ...
    public class RolesController : ApiController
    {
        ...
        public IHttpActionResult CreateRole([FromBody]Models.Dto.RolesDto roleDto)
        {
            try
            {
                if (string.IsNullOrEmpty(roleDto.StrValor) || string.IsNullOrEmpty(roleDto.StrDescripcion))
                {
                    return BadRequest("El nombre del rol y la descripción son obligatorios.");
                }
        ...
        [HttpPut]
        [Route("{id:int}")]
        public IHttpActionResult Update(int id, [FromBody] Models.Dto.RolesDto roleDto) // Cambio a [FromBody]
        {
            try
            {
                if (string.IsNullOrEmpty(roleDto.StrValor) || string.IsNullOrEmpty(roleDto.StrDescripcion))
